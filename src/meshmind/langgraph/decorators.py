    ...
    def decorator(node_func: Callable) -> Callable:
        ...
        async def wrapped_node(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            try:
                ...
                if hasattr(state, "get") and state.get(
                    "meshmind_graceful_degradation", True
                ):
                    logger.info(
                        "Proceeding with node execution despite intent generation failure"
                    )
                    return await node_func(state)
                else:
                    raise
...
def create_intent_node(
    intent_type: str, intent_payload_func: Callable[[Dict[str, Any]], Dict[str, Any]]
) -> Callable:
    """
...
def create_conditional_edge(
    decision_key: str = "intent_decision", default_node: str = "proceed"
) -> Callable:
    """
...
@asynccontextmanager
async def intent_context(intent_type: str, payload: Dict[str, Any]):
    """
