...
from .reports import GhostReport, StepReport, ConflictReport
...
class GhostSimulator:
    ...
    async def simulate(
        ...
        try:
            ...
            config = {"configurable": {"thread_id": f"ghost_{ghost_state.simulation_id}"}}
            ...
            async for step in self._simulate_steps(compiled_graph, ghost_state, config):
                ...
                if self.config.fail_on_conflict and step.conflicts:
                    logger.warning(f"Simulation stopped due to conflicts in step {step.step_number}")
                    break
                    
                if self.config.fail_on_budget_exceeded and step.budget_exceeded:
                    logger.warning(f"Simulation stopped due to budget exceeded in step {step.step_number}")
                    break
                    
                if ghost_state.step_count >= self.config.max_steps:
                    logger.warning(f"Simulation stopped at max steps ({self.config.max_steps})")
                    break
    ...
    async def _execute_node_ghost(
        ...
        async with BudgetContext(
            ...
            try:
                ...
                result["_ghost_budget_exceeded"] = ghost_state.budget_consumed > self.config.budget_cap
