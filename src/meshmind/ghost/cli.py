...
from .simulator import ghost_run, GhostConfig
...
@click.option('--fail-on-conflict', is_flag=True, help='Fail simulation on first conflict')
@click.option('--max-steps', default=100, help='Maximum execution steps')
@click.option('--config', '-c', type=click.Path(exists=True), help='Configuration file')
def run(graph_file: str, input_file: str, budget: float, rpm: int, 
        output: Optional[str], output_format: str, fail_on_conflict: bool, 
        max_steps: int, config: Optional[str]):
    """
...
def _load_graph_from_file(graph_file: str):
    ...
    raise ValueError(f"Could not find LangGraph workflow in {graph_file}. "
