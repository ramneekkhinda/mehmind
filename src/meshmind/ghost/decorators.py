...
async def _ghost_http_post(url: str, data: Dict[str, Any], **kwargs):
    ...
    estimated_cost = 0.001  # $0.001 per API call
...
async def _ghost_email_send(contact_id: str, body: str, subject: str, **kwargs):
    ...
    estimated_cost = 0.0001  # $0.0001 per email
...
def _estimate_llm_cost(provider: str, model: str, input_tokens: int, output_tokens: int) -> float:
    ...
    model_pricing = provider_pricing.get(model, provider_pricing.get('default', {'input': 0.001, 'output': 0.001}))
