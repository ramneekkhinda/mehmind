    ...
    def from_env(cls) -> "MeshMindConfig":
        ...
        return cls(
            ...
            fallback_on_error=os.getenv(
                "MESHMIND_FALLBACK_ON_ERROR", "true"
            ).lower() == "true",
            ...
            enable_ssl_verification=os.getenv(
                "MESHMIND_SSL_VERIFY", "true"
            ).lower() == "true"
        )
    ...
    def from_env(cls) -> "RefereeConfig":
        ...
        return cls(
            database_url=os.getenv(
                "DATABASE_URL", 
                "postgresql://meshmind:meshmind@localhost/meshmind"
            ),
            redis_url=os.getenv("REDIS_URL", "redis://localhost:6379"),
            policy_file=os.getenv("MESHMIND_POLICY_FILE", "policy.yaml"),
            policy_reload_interval=int(os.getenv(
                "MESHMIND_POLICY_RELOAD_INTERVAL", "300"
            )),
            max_connections=int(os.getenv("MESHMIND_MAX_CONNECTIONS", "20")),
            connection_timeout=float(os.getenv(
                "MESHMIND_CONNECTION_TIMEOUT", "30.0"
            )),
