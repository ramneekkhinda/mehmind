...
from meshmind.ghost import GhostConfig, GhostReport, GhostSimulator, ghost_run
...
class TestGhostConfig:
    ...
    def test_custom_config(self):
        ...
        config = GhostConfig(
            budget_cap=5.0, rpm_limit=30, fail_on_conflict=True, max_steps=50
        )
...
class TestGhostReport:
    ...
    def test_conflicts_by_type(self):
        ...
        conflicts = [
            ConflictReport(
                "resource_lock", "res1", "node1", 0, "high", "Lock conflict"
            ),
            ConflictReport(
                "frequency_cap", "res2", "node2", 1, "medium", "Frequency exceeded"
            ),
            ConflictReport(
                "resource_lock", "res3", "node3", 2, "low", "Another lock conflict"
            ),
        ]
    ...
    async def test_ghost_run_basic(self):
        ...
        with patch("meshmind.ghost.simulator.GhostSimulator") as mock_simulator_class:
            ...
            result = await ghost_run(
                mock_graph, {"test": "data"}, budget_cap=5.0, rpm_limit=30
            )
...
class TestStepReport:
    ...
    def test_step_report_creation(self):
        ...
        conflicts = [
            ConflictReport(
                "test_conflict", "test_resource", "test_node", 0, "low", "Test conflict"
            )
        ]
